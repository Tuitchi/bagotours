...
<body>
									...
									<tr>
										...
										switch ($row['status']) {
											...
											case "2":
												...
												break;
											case "3":
												echo "<button class='btn-waiting'>User Rating</button>";
												break;
											case "4":
	...
	<script>
		$(document).ready(function () {
				...
				$.getJSON(`../php/getBooking.php?id=${id}`)
					.done(function (data) {
						...
						<div class="btn-group">
							...
							` : `
								<a class="btn-success" href="../php/updateBooking.php?status=3&id=${data.book.booking_id}&user=${data.book.user_id}&tour=${data.book.tour_id}">Complete</a>
			...
			if (urlParams.has('id') && urlParams.get('complete') === 'true') {
				fetchBookingData(id, $('#completeContent'), '#completeModal');
			}
		});
	</script>
